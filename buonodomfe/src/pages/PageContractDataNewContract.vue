...
export default {
  ...
  methods: {
    ...
    richiedere (item) {
      ...
      this.tipo_intestatario = item.stepperForm.tipo_intestatario
    },
    allegare (item) {
      this.allegati = item
    },
    ...
    async settaIntestatario () {
      if (this.tipo_intestatario === "DESTINATARIO") {
        this.richiestaContratto.contratto.intestatario = this.utenteDestinatario;
      }

      if (this.tipo_intestatario === "RICHIEDENTE") {
        this.richiestaContratto.contratto.intestatario = this.utenteRichiedente;
      }

      if (this.tipo_intestatario !== "ALTRO") {
        delete this.richiestaContratto.contratto.intestatario.comune_residenza;
        delete this.richiestaContratto.contratto.intestatario.indirizzo_residenza;
        delete this.richiestaContratto.contratto.intestatario.provincia_residenza;
      }
    },
    async inviaContratto () {
      await store.dispatch("setSpinner", true);
      await this.controlloAllegati(this.allegati)
      await this.settaIntestatario()
      if (this.allegatiOk) {
        try {
