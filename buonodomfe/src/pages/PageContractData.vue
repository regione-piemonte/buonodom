    ...
    <div class="q-page lms-page">
      ...
      <template v-if="contrattiList && contrattiList.length > 0">
        <q-card class="q-mt-xl q-mb-xl">
          ...
          <q-card-section>
            <div class="row">
              <h3 class="col-12">Assistente familiare e contratto</h3>
            </div>
            <lms-banner type="info">
              <h2>
                Informazione su Assistente familiare e contratto
              </h2>
              <p>
            ...
            >
              <div>
                Intestatario
                <strong
                  >{{ item.contratto.intestatario.nome }}
                  {{ item.contratto.intestatario.cognome }}</strong
                >
              </div>
              <div>
                Data Inizio
              ...
              <div class="row q-mb-sm">
                <div class="col-md-9 col-sm-12">
                  ...
                  <strong
                    >{{ item.contratto.assistente_familiare.nome }}
                    {{ item.contratto.assistente_familiare.cognome }}</strong
                  >
                </div>
                <div
                  class="col-md-9 col-sm-12"
                  v-if="item.contratto.piva_assitente_familiare"
                >
                  P.IVA Assistente familiare:
                  <strong>{{ item.contratto.piva_assitente_familiare }}</strong>
                </div>
                <div
                  class="col-md-9 col-sm-12"
                  v-if="item.contratto.assistente_familiare.cf"
                >
                  CF Assistente familiare:
                  <strong>{{ item.contratto.assistente_familiare.cf }}</strong>
                </div>
                <div
                  class="col-12"
                ...
                <div class="col-12 q-mb-lg">
                  <q-btn
                    ...
                    outline
                    color="red"
                    :aria-label="'elimina il contratto numero '+ item.contratto.id + ' intestatario ' + item.contratto.intestatario.nome + ' ' + item.contratto.intestatario.cognome "
          ...
          <q-card-section>
            ...
            <lms-banner type="info">
              <p>
                Se hai
                <strong
                  >un altro contratto attivo della durata di almeno 12
                  mesi</strong
                >
                oltre a quello indicato, compila e invia il modulo apposito.
              </p>
            </lms-banner>
            <div class="row q-mb-sm">
              <div class="col-12">
...
export default {
  ...
  methods: {
    getChiusuraContrattoAriaLabel (item) {
      const nome = item.contratto.intestatario.nome;
      const cognome = item.contratto.intestatario.cognome;
      const dataInizio = moment(item.contratto.data_inizio).lang('it').format('DD MMMM YYYY');
      return `comunica chiusura contratto di: ${nome} ${cognome} - data inizio: ${dataInizio}`;
    },
