...
<template>
  <lms-page>
    ...
    <template v-else>
      <lms-page-title v-if="utenteRichiedente" class="q-mb-lg" no-back
        >Area personale di
        {{
      ...
      <template v-if="checkApi">
        <div class="q-pt-xl">
          ...
          <h2 v-if="listaRichieste.length">RICHIESTE ATTIVE</h2>
            ...
            <q-card class="q-mt-xl q-mb-xl q-card--background">
              <q-card-section>
                ...
                <div class="row justify-between items-end q-mt-md">
                  ...
                  <div
                    class="col-12 q-mt-xl  card-section--btn"
                    v-if="richiesta.stato == 'IN_PAGAMENTO' && richiesta.esistebuono"
                  >
  
                    <q-btn
                      @click="openApplication(richiesta.numero, 'gestione')"
                      color="primary"
                      :aria-label="
                        'gestione domanda numero: ' + richiesta.numero
                      "
                      label="GESTISCI IL BUONO"
                    ></q-btn>
                  ...
                  <div
                    class="col-12 q-mt-xl card-section--btn"
                    v-if="
                      richiesta.stato == 'AMMESSA_RISERVA_IN_PAGAMENTO' ||
                  ...
                  >
                    <q-btn
                      ...
                      color="primary"
                      :aria-label="
                        'perfeziona domanda numero: ' + richiesta.numero
                      "
                  ...
                  >
                    <q-btn
                      ...
                      color="primary"
                      :aria-label="
                        'modifica la bozza numero: ' + richiesta.numero
                      "
                      label="VAI ALLA BOZZA"
                    ></q-btn>

                    <q-btn
                      @click="settaStato('ANNULLATA', richiesta)"
                      :aria-label="
                        'elimina la bozza numero: ' + richiesta.numero
                      "
                      color="negative"
                      label="ELIMINA"
                    ></q-btn>
                  ...
                  >
                    <q-btn
                      ...
                      color="primary"
                      :aria-label="
                        'controdeduzione domanda numero: ' + richiesta.numero
                      "
                      label="CONTRODEDUZIONE DOMANDA"
                    ></q-btn>
                  </div>
                  <div
                    class="col-12 q-mt-xl card-section--btn"
                    v-if="
                      ...
                      richiesta.stato !== 'DINIEGO' &&
                      richiesta.stato !== 'RINUNCIATA'
                    "
                  >
                    <q-btn
                      v-if="
                        richiesta.stato == 'DA_RETTIFICARE' ||
                        richiesta.stato == 'IN_RETTIFICA'
                      "
                      @click="retifyApplication(richiesta, 'IN_RETTIFICA')"
                      color="primary"
                      :aria-label="
                        'rettifica la domanda numero: ' + richiesta.numero
                      "
                      label="RETTIFICA DOMANDA"
                    ></q-btn>

                    <q-btn v-if="!richiesta.esistebuono"
                      @click="openModalRenounce(richiesta)"
                      color="negative"
                      :aria-label="
                        'rinuncia alla domanda numero: ' + richiesta.numero
                      "
                      label="RINUNCIA DOMANDA"
                    ></q-btn>
                  </div>
                  <div
                    class="col-12 q-mt-xl card-section--btn"
                  ...
                  >
                    <a
                      class="btn--flat"
                      :href="url + '/' + richiesta.numero + '/DOMANDA'"
                      target="_blank"
                      :aria-label="
                        'scarica la richiesta numero ' + richiesta.numero
                      "
                    >
                      SCARICA RICHIESTA
                    </a>
        ...
        <template v-if="nuovaRichiestaBtn.length == 0">
          <div
            class="col-12 q-pt-xxl text-right"
            v-if="
              sportelliAttivi && sportelliAttivi.length > 0 && utenteRichiedente
            "
          >
        ...
        <template v-else>
          <lms-banner type="warning">
            <h2>Attenzione</h2>
            <p>
              Non Ã¨ possibile fare nuove richieste finchÃ¨ ci sono domande in
              stato "BOZZA"
            </p>
          </lms-banner>
        </template>
        ...
        <template v-if="!sportelliAttivi">
          <lms-banner type="warning">
            ...
            <p v-if="sportelliNonAttivi">
              SarÃ  possibile inviare una richiesta di Buono domiciliaritÃ  a
              partire dal
              <strong v-for="(sportello, i) in sportelliNonAttivi" :key="i">{{
                sportello[0].data_fine
              }}</strong>
            </p>
            <p v-else>Si prega di riprovare piÃ¹ tardi.</p>
          </lms-banner>
        </template>
            ...
            <q-card-section>
              <div class="row justify-between items-center">
                ...
                <div class="col text-right">
                  <a
                    ...
                    aria-label="modifica i tuoi contatti tornando al portale di Scelta sociale"
                    :href="linkPortale"
                    flat
                    target="_blank"
                  >
              ...
              <div class="q-pt-xl">
                <p>
                  ...
                  comunicazioni riguardanti il bonus domiciliaritÃ , verifica la
                  correttezza ed eventualmente aggiornali/modificali se ritieni
                  opportuno.
                </p>
              </div>
              <div
                class="row justify-between q-pt-xl items-start q-gutter-y-lg"
              >
                <div class="col-12 col-md">
      ...
      <template v-else
        ><lms-banner type="negative">
          <h2>Attenzione</h2>
          <p>Problemi server, contattare assistenza o riprovare piÃ¹ tardi</p>
        </lms-banner></template
      >
            ...
            <q-list>
              ...
              >
                <p class="q-mt-xl">
                  ...
                  domiciliaritÃ  occorre essere in possesso di una attestazione
                  ISEE socio-sanitario in corso di validitÃ  e di un verbale di
...
export default {
  ...
  async created () {
    await store.dispatch("setUtenteDestinatario", null);
    this.loading = true;
