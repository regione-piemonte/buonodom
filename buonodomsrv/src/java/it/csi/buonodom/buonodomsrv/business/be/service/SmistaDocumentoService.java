			...
			if (richiesta != null) {
				...
				for (ModelGetAllegatoExt allegato : allegati) {
					if (allegato.getCodTipoAllegato().equalsIgnoreCase(Constants.DOMANDA)) {
						domanda.setFileName(allegato.getFileName());
						domanda.setFileType(allegato.getFileType());
						domanda.setFilebyte(getAllegato(allegato.getFilePath(), allegato.getFileName()));
						domanda.setCodTipoAllegato(allegato.getCodTipoAllegato());
						domanda.setDescTipoAllegato(allegato.getDescTipoAllegato());
					} else {
				...
				if (allegati.size() > 0) {
					if (domanda != null && domanda.getFilebyte().length > 0) {
						...
						if (result.getResult().getCodice().equalsIgnoreCase(Constants.STARDAS_OK)
							...
							for (ModelGetAllegatoExt allegato : allegatialtro) {
								if (allegato.getFilebyte().length > 0) {
									...
									if (!result.getResult().getCodice().equalsIgnoreCase(Constants.STARDAS_OK)
										...
										logError(metodo, "Errore riguardante Stardas Invio Allegati:", param[0]);
										listerrorservice.add(validateGeneric
												.getValueGenericError(CodeErrorEnum.ERR03.getCode(), param));
										error = ErrorBuilder.generateErrorBuilderError(StatusEnum.SERVER_ERROR,
												listerrorservice);
							...
							param[0] = result.getResult().getMessaggio();
							logError(metodo, "Errore riguardante Stardas Invio Principale:", param[0]);
							listerrorservice
						...
						param[0] = "Allegato domanda non trovata " + numerorichiesta;
						logError(metodo, "Allegato domanda non trovata:", param[0]);
						listerrorservice
								.add(validateGeneric.getValueGenericError(CodeErrorEnum.ERR03.getCode(), param));
						error = ErrorBuilder.generateErrorBuilderError(StatusEnum.SERVER_ERROR, listerrorservice);
						Response esito = error.generateResponseError();
					...
					param[0] = "Allegati domanda non trovata " + numerorichiesta;
					logError(metodo, "Allegato domanda non trovata:", param[0]);
	...
	private SmistaDocumentoResponseType sendStardas(String tipoInvio, ModelGetAllegatoExt allegato,
		...
		try {
			...
			if (tipoInvio.equalsIgnoreCase(Constants.PRINCIPALE)) {
				...
				metadato.add(metadatosin);
				metadatosin = new MetadatoType();
				metadatosin.setNome(Constants.AUTORE_FISICO_DOCUMENTO);
				metadatosin.setValore(
						richiesta.getRichiedente().getCognome() + " " + richiesta.getRichiedente().getNome());
				metadato.add(metadatosin);
				metadatosin = new MetadatoType();
				metadatosin.setNome(Constants.MITTENTE_PF_COGNOME);
				metadatosin.setValore(richiesta.getRichiedente().getCognome());
				metadato.add(metadatosin);
				metadatosin = new MetadatoType();
				metadatosin.setNome(Constants.MITTENTE_PF_NOME);
				metadatosin.setValore(richiesta.getRichiedente().getNome());
				metadato.add(metadatosin);
				metadatosin = new MetadatoType();
				metadatosin.setNome(Constants.MITTENTE_PF_CODICE_FISCALE);
			...
			if (tipoInvio.equalsIgnoreCase(Constants.PRINCIPALE)) {
				...
				metadatosin.setValore(richiesta.getContributoTipoDesc() + " " + allegato.getCodTipoAllegato() + " "
				...
				metadatosin.setNome(Constants.OGGETTO_DOCUMENTO);
				metadatosin.setValore(allegato.getCodTipoAllegato());
				metadato.add(metadatosin);
				metadatosin = new MetadatoType();
				metadatosin.setNome(Constants.PAROLE_CHIAVE_DOCUMENTO);
				metadatosin.setValore(richiesta.getDomandaDetCod() + "_" + allegato.getCodTipoAllegato());
				metadato.add(metadatosin);
				metadatosin = new MetadatoType();
			}
