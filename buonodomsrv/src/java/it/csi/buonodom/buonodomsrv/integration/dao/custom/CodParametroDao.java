...
import it.csi.buonodom.buonodomsrv.dto.ModelParametro;
import it.csi.buonodom.buonodomsrv.exception.DatabaseException;
import it.csi.buonodom.buonodomsrv.integration.dao.utils.ListaParametroMapper;
import it.csi.buonodom.buonodomsrv.util.LoggerUtil;

@Repository
public class CodParametroDao extends LoggerUtil {

	public static final String SELECT_ERRORE_DESC = "SELECT a.parametro_valore FROM bdom_c_parametro a,bdom_c_parametro_tipo b where "
	...
	public static final String SELECT_PARAMETRO_PER_TIPO = "SELECT a.parametro_cod , a.parametro_valore FROM bdom_c_parametro a,bdom_c_parametro_tipo b where "
			+ "b.parametro_tipo_id = a.parametro_tipo_id and "
			+ "now()::date BETWEEN b.validita_inizio::date and COALESCE(b.validita_fine::date, now()::date) and b.data_cancellazione is null and "
			+ "now()::date BETWEEN a.validita_inizio::date and COALESCE(a.validita_fine::date, now()::date) and a.data_cancellazione is null and "
