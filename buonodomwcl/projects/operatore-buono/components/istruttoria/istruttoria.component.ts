...
export class IstruttoriaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  displayedColumns: string[] = ['numeroDomanda', 'Destinatario', 'Richiedente', 'codStato', 'dataDomanda1', 'enteGestore', 'statoEsitoDataVerifica', "dateDecorrenzaBuono", 'azione'];
    ...
    if (this.client.listaStatiSalvato.length == 0) {
      ...
      if (this.client.ricercaDomAperte.length > 0) {
        ...
        this.searchForm = this.fb.group({
          ...
          formStatoVerificaEnteGestore: [this.client.filtroDomandeAperte.statoVerificaEnteGestore],
  ...
  avviaRicerca() {
    ...
    let statoVerifica = this.searchForm.controls.statoVerifica.value && this.searchForm.controls.statoVerifica.value.toUpperCase() as string;
    if (statoVerifica !== null) {
      statoVerifica = statoVerifica.replace(/\s+/g, "_");
    } else {
      statoVerifica = null;
    }
    let verificaEnteEffettuata = this.searchForm.controls.formStatoVerificaEnteGestore.value && this.searchForm.controls.formStatoVerificaEnteGestore.value != "" ? this.searchForm.controls.formStatoVerificaEnteGestore.value.toUpperCase() : null;
    let esitoVerificaEnteGestore = null;
    const ricerca = new FiltriDomandeAperte(codSportello, numeroDomanda, statoDomanda, destinatario, richiedente, SECTION.RICERCA_DOMANDE_APERTE, this.client.listaIdEntiGestori, statoVerifica, verificaEnteEffettuata, esitoVerificaEnteGestore, null);
