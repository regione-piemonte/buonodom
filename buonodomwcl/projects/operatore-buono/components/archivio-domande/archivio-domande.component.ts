...
import { SECTION } from '@buonodom-app/constants/buonodom-constants';
...
export class ArchivioDomandeComponent implements OnInit, OnDestroy {
  ...
  constructor(private fb: FormBuilder, public client: BuonodomBOClient,
  ...
  ngOnInit() {
    ...
    if (this.client.listaStatiSalvatoArchivio.length == 0) {
      ...
      // Nel caso in cui siano già state effettuate delle ricerche
  ...
  // Funzione per ottenere le diverse proprietà di un oggetto
  ...
  avviaRicerca() {
    ...
    const ricerca = new FiltriDomandeAperte(codSportello, numeroDomanda, statoDomanda, destinatario, richiedente, SECTION.RICERCA_ARCHIVIO, null, null, null, null, null);
