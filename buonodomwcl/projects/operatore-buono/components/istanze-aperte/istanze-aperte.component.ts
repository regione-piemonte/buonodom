...
import { AZIONE, SECTION, STATO_DOMANDA } from '@buonodom-app/constants/buonodom-constants';
import { FiltriDomandeAperte } from '@buonodom-app/app/dto/FiltriDomandeAperte';
import { StatiBuonodom } from '@buonodom-app/app/dto/StatiBuonodom';
import { DomandeAperte } from '@buonodom-app/app/dto/DomandeAperte';
...
export class IstanzaAperteComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  displayedColumns: string[] = ['check', 'numeroDomanda', 'Destinatario', 'Richiedente', 'codStato', 'dataDomanda1', 'enteGestore', 'statoEsitoDataVerifica', 'azione'];
  ...
  constructor(private fb: FormBuilder, private router: Router, private dialog: MatDialog, private route: ActivatedRoute,
    private modalService: NgbModal, public client: BuonodomBOClient, public toastService: AppToastService) { }
  ...
  ngOnInit() {
    if (this.client.listaStatiSalvato.length == 0) {
      ...
      if (this.client.ricercaDomAperte.length > 0) {
        ...
        this.searchForm = this.fb.group({
          codSportello: [this.client.filtroDomandeAperte.codSportello, [Validators.required]],
          ...
          formVerificaEnteEffettuata: [this.client.filtroDomandeAperte.statoVerificaEnteGestore],
          formPresenzaDiMisureNonCompatibili: [this.client.filtroDomandeAperte.esitoVerificaEnteGestore],
          ultimaDataVerifica: [this.client.filtroDomandeAperte.ultimaDataVerifica],
        });
        this.matAutocomplete(); //MATAUTOCOMPLETE
      }
      else {
  ...
  avviaRicerca() {
    ...
    let statoVerifica = this.searchForm.controls.statoVerifica.value && this.searchForm.controls.statoVerifica.value != "" ? this.searchForm.controls.statoVerifica.value.toUpperCase() : null;
    let verificaEnteEffettuata = this.searchForm.controls.formVerificaEnteEffettuata.value && this.searchForm.controls.formVerificaEnteEffettuata.value != "" ? this.searchForm.controls.formVerificaEnteEffettuata.value.toUpperCase() : null;
    let esitoVerificaEnteGestore = this.searchForm.controls.formPresenzaDiMisureNonCompatibili.value && this.searchForm.controls.formPresenzaDiMisureNonCompatibili.value != "" ? this.searchForm.controls.formPresenzaDiMisureNonCompatibili.value.toUpperCase() : null;
    if (statoVerifica !== null) {
      statoVerifica = statoVerifica.replace(/\s+/g, "_");
    }
    let ultimaDataVerifica = this.searchForm.controls.ultimaDataVerifica.value;
    let dataItaliana = null;
    let dataFormattata = null;
    if (ultimaDataVerifica != null && ultimaDataVerifica != '') {
      dataItaliana = new Date(ultimaDataVerifica);
      dataFormattata = dataItaliana.toLocaleDateString("it-IT");
    }
    const ricerca = new FiltriDomandeAperte(codSportello, numeroDomanda, statoDomanda, destinatario, richiedente, SECTION.RICERCA_DOMANDE_APERTE, enteGestore, statoVerifica, verificaEnteEffettuata, esitoVerificaEnteGestore, dataFormattata);
    this.client.filtroDomandeAperte = ricerca;
