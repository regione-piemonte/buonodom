...
import { Feedback } from '@buonodom-app/app/dto/Feedback';
import { ModelRichiesta } from '@buonodom-app/app/dto/ModelRichiesta';
import { ALLEGATI, AZIONE, DATIENTE_INFO, STATO_DOMANDA, TIPO_FEEDBACK, VALUTAZIONE } from '@buonodom-app/constants/buonodom-constants';
...
import { ModelVerificheEnte } from '@buonodom-app/app/dto/ModelVerificheEnte';
import { ValidaDati } from '@buonodom-app/shared/controlli/validadati';
import { ModelVisualizzaVerifiche } from '@buonodom-app/app/dto/ModelVisualizzaVerifiche';
...
export class DatiIstanzaEnteComponent implements OnInit {
  ...
  urlTutela: string;
  urlContratto: string;
  urlContrattoCoop: string;
  urlControdeduzione: string;
  urlDenunciaInps: string;
  ...
  constructor(private router: Router, private route: ActivatedRoute, private fb: FormBuilder, private dialog: MatDialog,
    public client: BuonodomBOClient, private modalService: NgbModal, public toastService: AppToastService, private buonodomError: BuonodomErrorService) {
  ...
  ngOnInit() {
    ...
    forkJoin({
      ...
      .subscribe(({ schedaDomanda, verificheCronologia }) => {
        this.client.azioni;
        ...
        if (this.domanda.allegati.findIndex(element => element.tipo == ALLEGATI.DENUNCIA_INPS) != -1) {
          this.denunciainps = this.domanda.allegati.filter(element => element.tipo == ALLEGATI.DENUNCIA_INPS)[0].filename;
          this.urlDenunciaInps = this.client.getAllegato(this.domanda.numero, ALLEGATI.DENUNCIA_INPS);
        }
        if (this.domanda.allegati.findIndex(element => element.tipo == ALLEGATI.CONTRATTO_LAVORO) != -1) {
          this.contratto = this.domanda.allegati.filter(element => element.tipo == ALLEGATI.CONTRATTO_LAVORO)[0].filename;
          this.urlContratto = this.client.getAllegato(this.domanda.numero, ALLEGATI.CONTRATTO_LAVORO);
        }
        if (this.domanda.allegati.findIndex(element => element.tipo == ALLEGATI.CONTRATTO_LAVORO_COOP) != -1) {
          this.contrattocoop = this.domanda.allegati.filter(element => element.tipo == ALLEGATI.CONTRATTO_LAVORO_COOP)[0].filename;
          this.urlContrattoCoop = this.client.getAllegato(this.domanda.numero, ALLEGATI.CONTRATTO_LAVORO_COOP);
        }
  ...
  checkAzione(azione: string) {
    ...
    switch (azione) {
      ...
      case "salva_verifica_ente":
        return this.client.azioni.some(a => a.codAzione == AZIONE.OP_Salva_Verifica_Ente);
      ...
      case "concludi_verifica_ente":
        return this.client.azioni.some(a => a.codAzione == AZIONE.OP_Concludi_Verifica_Ente);
      case "verifiche":
  ...
  abilitabottoni() {
    ...
    //Avvia Verifica
    if (this.checkStato() && this.domanda != undefined && this.domanda.verifiche.verifica_eg_richiesta && !(this.domanda.verifiche.verifica_eg_in_corso || this.domanda.verifiche.verifica_eg_conclusa)) {
    ...
    //Concludi Verifica
    if (this.checkStato() && this.domanda != undefined && this.domanda.verifiche.verifica_eg_in_corso && !(this.domanda.verifiche.verifica_eg_richiesta || this.domanda.verifiche.verifica_eg_conclusa)) {
    ...
    //Salva Verifica
    if (this.checkStato() && this.domanda != undefined && !(this.domanda.verifiche.verifica_eg_richiesta || this.domanda.verifiche.verifica_eg_in_corso)) {
  ...
  salvaVerificaEnte() {
    ...
    if (this.conformeOption == true)
      ...
      this.domanda.verifiche.verifica_eg_incompatibilita = null;

    let verifica: ModelVerificheEnte = new ModelVerificheEnte(this.conformeOption, this.verificheEnteGestore.noteEnte, this.verificheEnteGestore.dataVerifica);
    this.client.salvaverificaente(verifica, this.numerodomanda).subscribe(res => {
      if (res) {
  ...
  avviaVerificaEnte() {
    ...
    this.client.updateToVerificaInCorso(this.numeroDomande).subscribe(res => {
      if (res) {
  ...
  concludiVerificaEnte() {
    ...
    dialogRef.afterClosed().subscribe((result: boolean) => {
      if (result) {
        ...
        let verifica: ModelVerificheEnte = new ModelVerificheEnte(this.conformeOption, this.verificheEnteGestore.noteEnte, null);
  ...
  checkStato() {
    return this.domanda != null && (this.domanda.stato != STATO_DOMANDA.RINUNCIATA && this.domanda.stato != STATO_DOMANDA.DINIEGO && this.domanda.stato != STATO_DOMANDA.ANNULLATA);
  }
  ...
  getVerificheEnte() {
    this.client.getVerificheEnte(this.numerodomanda).subscribe(
      (response: ModelVerificheEnte) => {
        ...
        this.conformeOption = this.verificheEnteGestore.presenzaAltreIncopatibilita;
      },
      err => {
        this.client.spinEmitter.emit(false);
      }
