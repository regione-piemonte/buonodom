...
******************************************************************************/
package it.csi.buonodom.buonodombandisrv.filter;
 ...
 * di tipo XSRF. L'idea e' che il primo accesso avvenga sulla risorsa di accesso
 * della componente SPA (es. index.html). In quell'occasione il filtro non fa
 * verifiche ma si preoccupa di creare una nuova sessione servlet se non
 * presente, creare un XSRF cookie, metterlo in sessione e passarlo al client.
 * Nelle successive richieste, se la rihciesta e' destinata allo strato di
 * servizi rest, viene effettuata la verifica di presenza/corrispondenza di
 * cookie+header XSRF. Questo comportamento, unito alla corretta gestione client
 * side del protocollo, permette di evitare il richiamo dello strato rest al di
 * fuori del contesto corretto.
 */
public class XSRFProtectionFilter implements Filter {
	 ...
	 * <ul>
	 * <li>viene creato un nuovo token che viene inserito in sessione e restituito
	 * come cookie</li>
	 * </ul>
	 * </li>
	 * <li>se la richiesta corrente e' destinata alla parte REST:
	 * <ul>
	 * <li>se la request contiene una coppia di cookie e header XSRF validi e
	 * coincidenti con quello corrente (contenuto in sessione) => procedo,
	 * altrimenti errore.</li>
	 * </ul>
	 * </li>
	 * </ul>
	 */
	...
	/**
	 * E' necessario fare in modo che le risorse (pagine, etc) non siano mantenute
	 * nella cache del browser per evitare che, ad un accesso in una sessione
	 * successiva, la fase di ingresso (e quindi di ricreazione del token) venga
	 * bypassata. Se cio' accadesse le richieste REST sarebbero le prime ad arrivare
	 * al server e si ricadrebbe nella casistica di token non ancora inizializzato.
	 * 
	 * @param response
	 */
	 ...
	 *         dell'applicazione che per default deve contenere la stringa
	 *         "restfacade"
	 */
	 ...
	 * 
	 * @param hreq la request
	 * @return il valore del cookie XSRF, se presente nella request; la stringa
	 *         vuota se invece non presente.
	 */
	 ...
	 * meccanismo. Serve negli scenari di sviluppo in cui la parte client non e'
	 * deployata nell'ear dei servizi
	 */
