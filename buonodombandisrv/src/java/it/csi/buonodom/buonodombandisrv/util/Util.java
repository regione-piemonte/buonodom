...
public class Util {
	...
	public static boolean isAgeBetween18and65(String data) {
	...
	public static String nomeMese(Integer mese) {
		Map<Integer, String> elencomesi = new HashMap<Integer, String>();
		elencomesi.put(1, "GENNAIO");
		elencomesi.put(2, "FEBBRAIO");
		elencomesi.put(3, "MARZO");
		elencomesi.put(4, "APRILE");
		elencomesi.put(5, "MAGGIO");
		elencomesi.put(6, "GIUGNO");
		elencomesi.put(7, "LUGLIO");
		elencomesi.put(8, "AGOSTO");
		elencomesi.put(9, "SETTEMBRE");
		elencomesi.put(10, "OTTOBRE");
		elencomesi.put(11, "NOVEMBRE");

		return elencomesi.get(mese);
	...
	public static int getNumeroMesiDifferenzaDate(Date dataDa, Date dataA) {
		...
		if (meseDa <= meseA) {
			...
			// se il mese di arrivo è precedente a quello di inizio allora calcolo la
			// differenza in mesi come il mese di arrivo + 12 (mesi in un'anno) - mese di
			// inizio
			int tempDiff = 12 - meseDa;
			diffMesi = tempDiff + meseA;
			if (diffAnni != 0) {
				// se gli anni sono diversi devo sottrarre 1 alla loro differenza perchè un
				// anno lo uso per il calcolo dei mesi di differenza
				diffAnni -= 1;
			}
		}
