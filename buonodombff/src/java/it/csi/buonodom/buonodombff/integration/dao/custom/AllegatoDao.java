...
import it.csi.buonodom.buonodombff.integration.dao.utils.AllegatoExtMapper;
...
public class AllegatoDao extends LoggerUtil {
	...
	public static final String SELECT_DET_COD_ALLEGATO = "select count(*) from bdom_t_allegato a "
			+ " where a.sportello_id = :idSportello " + " and a.domanda_det_id = :idDomanda"
			+ " and a.domanda_det_cod = :detCod"
			+ " and a.allegato_tipo_id = (Select s.allegato_tipo_id from bdom_d_allegato_tipo s where s.allegato_tipo_cod = :tipo)";

	public static final String UPDATE_DOMANDA_ALLEGATO = "UPDATE bdom_t_allegato a "
			+ "SET data_modifica=now(),utente_modifica=:cfmodifica, file_name=:nomefile, file_type=:tipofile, file_path=:pathfile "
			+ "from bdom_t_domanda_dettaglio b " + "WHERE b.domanda_det_cod = a.domanda_det_cod "
			+ "and b.domanda_det_id = a.domanda_det_id " + "and a.sportello_id = b.sportello_id "
			+ "and a.domanda_det_cod =:detcod "
			+ "and a.allegato_tipo_id = (Select s.allegato_tipo_id from bdom_d_allegato_tipo s where s.allegato_tipo_cod = :cod)  ";

	public static final String INSERT_DOMANDA_ALLEGATO = "INSERT INTO bdom_t_allegato "
			+ "(file_name, file_type, file_path, sportello_id, " + "domanda_det_id, domanda_det_cod, allegato_tipo_id, "
			+ "utente_creazione, utente_modifica) "
			+ " VALUES(:file_name, :file_type, :file_path, :sportello_id, :domanda_det_id, :domanda_det_cod, "
			+ "(select allegato_tipo_id from bdom_d_allegato_tipo where allegato_tipo_cod = :allegatotipocod), "
			+ ":utente_creazione, :utente_modifica)";

	public static final String SELECT_ALLEGATO = "select bta.file_path,bta.file_name  " + " from bdom_t_allegato bta  "
			+ " where  " + " bta.domanda_det_id = :domandaID " + " and bta.domanda_det_cod = :detcoD  "
			+ " and bta.data_cancellazione is null "
			+ " and bta.allegato_tipo_id = (select bdat.allegato_tipo_id  from bdom_d_allegato_tipo bdat where bdat.allegato_tipo_cod = :allegatoCod) ";

	public static final String UPDATE_ALLEGATO_DOMANDA_ID = "UPDATE bdom_t_allegato bta  "
			+ " set  bta.data_modifica = now(), bta.utente_modifica = :cf, bta.data_cancellazione = now()  "
			+ " where  " + " bta.domanda_det_id = :domandaId ";

	public static final String SELECT_GET_ALLEGATO = "select bta.file_path,bta.file_name  "
			+ " from bdom_t_allegato bta  " + " where  " + " bta.domanda_det_id = :domandaID "
			+ " and bta.data_cancellazione is null "
			+ " and bta.allegato_tipo_id = (select bdat.allegato_tipo_id  from bdom_d_allegato_tipo bdat where bdat.allegato_tipo_cod = :allegatoCod) ";
	...
	public static final String SELECT_ALLEGATI = "select bta.file_path,bta.file_name,bta.allegato_id,bta.sportello_id,bta.allegato_tipo_id,"
			+ " bta.file_type, a.allegato_tipo_cod " + " from bdom_t_allegato bta, bdom_d_allegato_tipo a  "
	...
	public static final String INSERT_DOMANDA_ALLEGATO_ID_TIPO_ALLEGATO = "INSERT INTO bdom_t_allegato "
			+ "(file_name, file_type, file_path, sportello_id, " + "domanda_det_id, domanda_det_cod, allegato_tipo_id, "
			+ "utente_creazione, utente_modifica) "
			+ " VALUES(:file_name, :file_type, :file_path, :sportello_id, :domanda_det_id, :domanda_det_cod, "
			+ ":allegatoTipoId, " + ":utente_creazione, :utente_modifica)";

	public static final String UPDATE_ALLEGATO_DATA_CANCELLAZIONE_BY_ID_DOMANDA = "UPDATE bdom_t_allegato  "
			+ " set  data_modifica = now(), utente_modifica = :cf, data_cancellazione = now()  " + " where  "
			+ " domanda_det_id = (select t.domanda_det_id "
			+ "		from bdom_t_domanda_dettaglio t where t.domanda_det_cod = :detCod) "
			+ " and allegato_tipo_id = (select t.allegato_tipo_id "
			+ "from bdom_d_allegato_tipo t where t.allegato_tipo_cod = :tipoAllegato) ";
