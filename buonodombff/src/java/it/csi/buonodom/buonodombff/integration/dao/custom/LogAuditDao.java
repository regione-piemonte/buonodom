...
import it.csi.buonodom.buonodombff.dto.custom.LogAudit;
...
public class LogAuditDao extends LoggerUtil {
	...
	public static final String INSERT_LOG_AUDIT = "insert into csi_log_audit "
			...
			+ "pgp_sym_encrypt(:request_payload, '@keyEncrypt@')::bytea, pgp_sym_encrypt(:response_payload, '@keyEncrypt@')::bytea, :esito_chiamata)";
	...
	public long insertLogAudit(LogAudit logAudit) throws DatabaseException {
		...
		if (Util.isValorizzato(logAudit.getRequestPayload()))
			params.addValue("request_payload", logAudit.getRequestPayload(), Types.VARCHAR);
		else
			params.addValue("request_payload", null, Types.VARCHAR);
		if (Util.isValorizzato(logAudit.getResponsePayload()))
			params.addValue("response_payload", logAudit.getResponsePayload(), Types.VARCHAR);
		else
			params.addValue("response_payload", null, Types.VARCHAR);
