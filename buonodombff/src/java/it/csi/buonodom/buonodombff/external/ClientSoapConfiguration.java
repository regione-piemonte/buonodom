...
******************************************************************************/
package it.csi.buonodom.buonodombff.external;
...
import it.csi.buonodom.buonodombff.util.log.LogEventSenderInCustom;
import it.csi.buonodom.buonodombff.util.log.LogEventSenderOutCustom;
...
public class ClientSoapConfiguration {
	...
	public JaxWsProxyFactoryBean proxyFactoryBeanInterrogaMef() {
		...
		proxyFactory.setAddress(interrogaMefServiceUrl);
	...
	public SoapInterceptor createInterrogaMefSecurityInterceptor() {
		...
		outProps.put(WSHandlerConstants.USER, this.interrogaMefUserBe);
			...
			public void handle(Callback[] callbacks) throws IOException, UnsupportedCallbackException {
				for (Callback callBack : callbacks) {
					if (callBack instanceof WSPasswordCallback) {
						((WSPasswordCallback) callBack).setPassword(interrogaMefPassBe);
					}
				}
			}
		});
