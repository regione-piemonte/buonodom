...
import it.csi.buonodom.buonodombo.integration.dao.custom.BatchDao;
...
public class BatchService extends BaseService {
	...
	public Long inserisciBatchOK(String numeroDomanda, String batchCode, String motivoCod, String utente,
		...
		try {
			...
			if (modelRichiestaBatch != null) {
				...
				Integer numtentativi = batchDao.selectNumeroTentativiOK(modelRichiestaBatch, motivoCod);
				if (numtentativi == 0) {
	...
	public Long inserisciBatchKO(String numeroDomanda, String batchCode, String motivoCod, String utente,
		...
		try {
			...
			if (modelRichiestaBatch != null) {
				...
				Integer tentativi = batchDao.selectBatchPrametro(batchCode, "NUMERO_TENTATIVI");
				// select numero dela batch
				Integer numtentativi = batchDao.selectNumeroTentativiKO(modelRichiestaBatch, motivoCod);
				if (numtentativi == 0) {
	...
	public Long inserisciBatchStepOK(String batchesecdetStep, String batchesecdetNote, Long batchesecId,
		...
		try {
			result = batchDao.insertBatchEsecuzioneStepOK(batchesecdetStep, batchesecdetNote, batchesecId,
					batchDao.selectStatoBatch("STATO_OK"), utente);
			log.info(metodo + " - Fine ");
		} catch (DatabaseException e) {
			log.error(metodo + " - errore " + e.getMessage());
			e.printStackTrace();
	...
	public Long selectEventoKo(String numeroDomanda, String batcheCodStato) {
		String metodo = new Object() {
		}.getClass().getEnclosingMethod().getName();
		log.info(metodo + " - Inizio ");
		Long result = null;
		try {
			...
			modelRichiestaBatch = batchDao.selectDomandaBatch(numeroDomanda);
			if (modelRichiestaBatch != null) {
				result = batchDao.selectEsisteAvviatoKo(batcheCodStato, modelRichiestaBatch);
				log.info(metodo + " - Fine ");
			}
		} catch (DatabaseException e) {
			log.error(metodo + " - errore " + e.getMessage());
	...
	public Long selectEventoOk(String numeroDomanda, String batcheCodStato) {
		String metodo = new Object() {
		}.getClass().getEnclosingMethod().getName();
		log.info(metodo + " - Inizio ");
		Long result = null;
		try {
			...
			modelRichiestaBatch = batchDao.selectDomandaBatch(numeroDomanda);
			if (modelRichiestaBatch != null) {
				result = batchDao.selectEsisteAvviatoOk(batcheCodStato, modelRichiestaBatch);
				log.info(metodo + " - Fine ");
			}
		} catch (DatabaseException e) {
			log.error(metodo + " - errore " + e.getMessage());
