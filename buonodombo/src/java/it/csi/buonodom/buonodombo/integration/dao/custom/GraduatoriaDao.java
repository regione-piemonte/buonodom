...
import it.csi.buonodom.buonodombo.integration.dao.utils.AreaMapper;
import it.csi.buonodom.buonodombo.integration.dao.utils.DescrizioneGraduatoriaMapper;
import it.csi.buonodom.buonodombo.integration.dao.utils.DomandeAmmissibiliMapper;
import it.csi.buonodom.buonodombo.integration.dao.utils.DomandeInGraduatoriaMapper;
import it.csi.buonodom.buonodombo.integration.dao.utils.GraduatoriaOrdinamentoMapper;
import it.csi.buonodom.buonodombo.integration.dao.utils.ParametriFinanziamentoMapper;
...
public class GraduatoriaDao extends LoggerUtil {
	...
	private static String SELECT_ULTIMO_SPORTELLO_CHIUSO = "select bts.sportello_cod, bts.sportello_desc, false is_corrente, "
			+ " to_char(bts.validita_inizio,'YYYY-MM-DD') as validita_inizio, "
			+ " to_char(bts.validita_fine,'YYYY-MM-DD') as validita_fine, " + " bts.sportello_anno "
	...
	private static String SELECT_GRADUATORIA_ORDINAMENTO = "select bdgo.ordinamento_id, bdgo.ordinamento_criterio, "
			+ "	bdgo.ordinamento_tabella, bdgo.ordinamento_campo, "
			+ "	bdgo.ordinamento_posizione, bdgo.ordinamento_direzione " + "from bdom_d_graduatoria_ordinamento bdgo "
	...
	private static String SELECT_AREE = "select area_id, area_cod , area_desc " + "from bdom_d_area "
			+ "where data_cancellazione is null "
			+ "and now()::date between validita_inizio::date and coalesce(validita_fine::date, now()::date) "
			+ "order by area_id ";

	private static String INSERT_T_GRADUATORIA = "INSERT INTO bdom_t_graduatoria (sportello_id, graduatoria_desc , utente_creazione, utente_modifica) "
			+ "VALUES ((select sportello_id  " + "	from bdom_t_sportello " + "	where data_cancellazione is null "
			+ "	and sportello_cod  = :codSportello), :descGraduatoria , :utenteCreazione, :utenteModifica) ";

	private static String INSERT_R_GRADUATORIA_STATO = "INSERT INTO bdom_r_graduatoria_stato (graduatoria_id, graduatoria_stato_id, utente_creazione, utente_modifica) "
	...
	private static String INSERT_R_GRADUATORIA_ORDINAMENTO = "INSERT into bdom_r_graduatoria_ordinamento (graduatoria_id, ordinamento_id, utente_creazione, utente_modifica) "
			+ "values (:idGraduatoria,:idOrdinamento, :utenteCreazione, :utenteModifica) ";

	private static String INSERT_T_GRADUATORIA_FINANZIAMENTO = "insert into bdom_t_graduatoria_finanziamento (finanziamento_desc, finanziamento_priorita, "
	...
	private static String UPDATE_FINANAZIAMENTO_GRADUATORIA = "UPDATE bdom_t_graduatoria_finanziamento "
			+ "SET finanziamento_id_destinazione  = (case "
			+ "	when coalesce(area_id,0) !=0 then (finanziamento_id+3-area_id) " + "	else null " + "end) "
			+ "where finanziamento_id = ( " + "	select finanziamento_id  from bdom_t_graduatoria_finanziamento btgf  "
			+ "	where graduatoria_id = :idGraduatoria " + "	and coalesce(area_id,0) = coalesce(:idArea,0)); " + " "
			+ "UPDATE bdom_t_graduatoria_finanziamento " + "SET finanziamento_importo_ricevuto = (case "
			+ "	when coalesce(area_id,0) !=0 then 0 "
			+ "	else (select sum(coalesce(finanziamento_importo_distribuito ,0))  "
			+ "	from bdom_t_graduatoria_finanziamento btgf  " + "	WHERE graduatoria_id = :idGraduatoria "
			+ "	and coalesce(area_id,0) !=0) " + "end) " + "where finanziamento_id = ( "
			+ "	select finanziamento_id  from bdom_t_graduatoria_finanziamento btgf  "
			+ "	where graduatoria_id = :idGraduatoria " + "	and coalesce(area_id,0) = coalesce(:idArea,0)); " + " "
			+ "UPDATE bdom_t_graduatoria_finanziamento " + "set finanziamento_importo_finanziabile = (case "
			+ "	when coalesce(area_id,0) !=0 then finanziamento_importo_totale "
			+ "	else (coalesce(finanziamento_importo_totale ,0)+coalesce(finanziamento_importo_ricevuto,0)) " + "end) "
			+ "where finanziamento_id = ( " + "	select finanziamento_id  from bdom_t_graduatoria_finanziamento btgf  "
			+ "	where graduatoria_id = :idGraduatoria " + "	and coalesce(area_id,0) = coalesce(:idArea,0)); " + " "
			+ "UPDATE bdom_t_graduatoria_finanziamento " + "set numero_domande_finanziate  = ( " + "case  "
			+ "	when coalesce((select count(*)*(select (finanziamento_mesi*finanziamento_importo_mensile)  "
			+ "	from bdom_t_graduatoria_finanziamento  " + "	where graduatoria_id = :idGraduatoria "
			+ "	and coalesce(area_id,0) = coalesce(:idArea,0)) " + "		from bdom_t_graduatoria_dettaglio btgd  "
			...
			+ "	then (select count(*) " + "		from bdom_t_graduatoria_dettaglio btgd  "
			+ "		where btgd.finanziamento_id = ( "
			+ "			select finanziamento_id  from bdom_t_graduatoria_finanziamento btgf  "
			+ "			where graduatoria_id = :idGraduatoria "
			...
			+ "	select finanziamento_id  from bdom_t_graduatoria_finanziamento btgf  "
			+ "	where graduatoria_id = :idGraduatoria " + "	and coalesce(area_id,0) = coalesce(:idArea,0)); " + " "
			+ " " + "UPDATE bdom_t_graduatoria_finanziamento "
			+ "set finanziamento_importo_finanziato = (numero_domande_finanziate * finanziamento_mesi * finanziamento_importo_mensile) "
			+ "where finanziamento_id = ( " + "	select finanziamento_id  from bdom_t_graduatoria_finanziamento btgf  "
			+ "	where graduatoria_id = :idGraduatoria " + "	and coalesce(area_id,0) = coalesce(:idArea,0)); " + " "
			+ "UPDATE bdom_t_graduatoria_finanziamento " + "set finanziamento_importo_residuo=(  "
			...
			+ "	select finanziamento_id  from bdom_t_graduatoria_finanziamento btgf  "
			+ "	where graduatoria_id = :idGraduatoria " + "	and coalesce(area_id,0) = coalesce(:idArea,0)); " + " "
			+ "UPDATE bdom_t_graduatoria_finanziamento " + "set finanziamento_importo_distribuito = (case "
	...
	private static String SELECT_DOMANDE_AMMISSIBILI = "select btdd.domanda_det_id, btd.domanda_numero, btd.richiedente_cf, btdd.sportello_id, "
			+ "	case " + "		when btdd.nessuna_incompatibilita = false  "
			+ "			or btdd.incompatibilita_per_contratto = true  " + "		then true else false "
			+ "	end as riserva " + "from bdom_t_domanda btd, bdom_t_domanda_dettaglio btdd "
			+ "where btd.domanda_id = btdd.domanda_id  " + "and btdd.domanda_stato_id = ( "
			+ "	select bdds.domanda_stato_id  " + "	from bdom_d_domanda_stato bdds  "
			+ "	where bdds.domanda_stato_cod  = (:stato) " + ") " + "and btdd.sportello_id  = ( "
			+ "	select bts.sportello_id  " + "	from bdom_t_sportello bts "
			+ "	where bts.sportello_cod = :codSportello " + "	and bts.data_cancellazione is null  " + ") "
	...
	private static String SELECT_DOMANDE_AMMESSE_SPORTELLI = "select domanda_det_id "
			+ "from bdom_t_domanda_dettaglio, bdom_t_domanda "
			+ "where bdom_t_domanda_dettaglio.data_cancellazione is null "
			+ "and bdom_t_domanda.data_cancellazione is null "
			+ "and bdom_t_domanda.domanda_id = bdom_t_domanda_dettaglio.domanda_id "
			+ "and now() between bdom_t_domanda_dettaglio.validita_inizio and coalesce(bdom_t_domanda_dettaglio.validita_fine, now()) "
			+ "and bdom_t_domanda_dettaglio.domanda_stato_id in ( " + "	select bdds.domanda_stato_id  "
			+ "	from bdom_d_domanda_stato bdds " + "	where bdds.data_cancellazione is null "
			...
			+ "and bdom_t_domanda_dettaglio.sportello_id in( " + "	select bts.sportello_id  "
			+ "	from bdom_t_sportello bts " + "	where bts.data_cancellazione is null " + "	and bts.validita_fine <= ( "
			+ "		select bts2.validita_fine " + "		from bdom_t_sportello bts2 "
			+ "		where data_cancellazione is null " + "		and bts2.sportello_cod = :codSportello " + " ) " + ") ";

	private static String DELETE_GRADUATORIA = "delete from bdom_t_graduatoria_dettaglio "
			+ "			where data_cancellazione is null " + "			and graduatoria_id  in ( "
			+ "			select graduatoria_id from bdom_t_graduatoria  btg "
			+ "			where btg.data_cancellazione is null " + "			and btg.sportello_id in ( "
			+ "				select bts.sportello_id " + "				from bdom_t_sportello bts "
			+ "				where bts.data_cancellazione is null "
			+ "				and bts.sportello_cod = :codSportello)); " + " "
			+ " 		delete from bdom_r_graduatoria_ordinamento " + "			where data_cancellazione is null "
			+ "			and graduatoria_id  in ( " + "			select graduatoria_id from bdom_t_graduatoria  btg "
			+ "			where btg.data_cancellazione is null " + "			and btg.sportello_id in ( "
			+ "				select bts.sportello_id " + "				from bdom_t_sportello bts "
			+ "				where bts.data_cancellazione is null "
			+ "				and bts.sportello_cod = :codSportello)); " + " "
			+ " 		delete from bdom_r_graduatoria_stato " + "			where data_cancellazione is null "
			+ "			and graduatoria_id  in ( " + "			select graduatoria_id from bdom_t_graduatoria  btg "
			+ "			where btg.data_cancellazione is null " + "			and btg.sportello_id in ( "
			+ "				select bts.sportello_id " + "				from bdom_t_sportello bts "
			+ "				where bts.data_cancellazione is null "
			+ "				and bts.sportello_cod = :codSportello)); " + " "
			+ "         delete from bdom_t_graduatoria_finanziamento " + "			where data_cancellazione is null "
			+ "			and graduatoria_id  in ( " + "			select graduatoria_id from bdom_t_graduatoria  btg "
			+ "			where btg.data_cancellazione is null " + "			and btg.sportello_id in ( "
			+ "				select bts.sportello_id " + "				from bdom_t_sportello bts "
			+ "				where bts.data_cancellazione is null "
			+ "				and bts.sportello_cod = :codSportello)); " + " " + " 		delete from bdom_t_graduatoria "
			+ "			where data_cancellazione is null " + "			and graduatoria_id  in ( "
			+ "			select graduatoria_id from bdom_t_graduatoria  btg "
			+ "			where btg.data_cancellazione is null " + "			and btg.sportello_id in ( "
			+ "				select bts.sportello_id " + "				from bdom_t_sportello bts "
	...
	private static String INSERT_T_GRADUATORIA_DETTAGLIO = "insert into bdom_t_graduatoria_dettaglio (graduatoria_id, domanda_id, sportello_id, "
			+ "finanziamento_id, posizione, importo, " + "utente_creazione, utente_modifica) "
			+ "values ((select graduatoria_id from bdom_t_graduatoria btg "
			+ "			where btg.data_cancellazione is null " + "			and btg.sportello_id in ( "
			+ "				select bts.sportello_id " + "				from bdom_t_sportello bts "
			+ "				where bts.data_cancellazione is null "
			+ "				and bts.sportello_cod = :codSportello)), "
			+ "(select domanda_id from bdom_t_domanda_dettaglio btdd where btdd.domanda_det_id = :idDettaglio), "
			+ "(select sportello_id from bdom_t_domanda_dettaglio btdd where btdd.domanda_det_id = :idDettaglio), "
			+ "(select finanziamento_id  " + "from bdom_t_graduatoria_finanziamento btgf "
			+ "where graduatoria_id = (select graduatoria_id from bdom_t_graduatoria btg "
			+ "			where btg.data_cancellazione is null " + "			and btg.sportello_id in ( "
			+ "				select bts.sportello_id " + "				from bdom_t_sportello bts "
			+ "				where bts.data_cancellazione is null "
			+ "				and bts.sportello_cod = :codSportello)) " + "and coalesce(btgf.area_id, 0)  = coalesce(( "
			+ "	SELECT area_id " + "	FROM bdom_t_domanda_dettaglio btdd "
			+ "	WHERE btdd.domanda_det_id = :idDettaglio), 0)), " + ":posizione, " + ":importo, " + ":utenteCreazione, "
			+ ":utenteModifica " + ") ";

	private static String SELECT_DOMANDE_IN_GRADUATORIA = " select btgd.posizione, btd.domanda_numero,   "
			...
			+ "			 	initcap(replace(bdds.domanda_stato_cod, '_', ' ')) as domanda_stato_cod ,btgd.importo, "
			+ "             btdd.verifica_eg_richiesta, btdd.verifica_eg_in_corso, btdd.verifica_eg_conclusa, "
			+ "			 	case    " + "			 		when btdd.area_id is not null  "
			+ "			 		then ( " + "			 			select bda.area_desc  "
			+ "			 			from bdom_d_area bda " + "			 			where bda.area_id = btdd.area_id "
			+ "			 		)   " + "			 		when btdd.area_id is null  "
			+ "			 		then null  " + "			 		else null   " + "			 	end as area   "
			+ "			 from bdom_t_graduatoria_dettaglio btgd, bdom_t_domanda btd,    "
			+ "			 	bdom_t_domanda_dettaglio btdd, bdom_d_domanda_stato bdds    "
	...
	private static String SELECT_PARAMETRI_FINANZIAMENTO_GRADUATORIA = "select btgf.finanziamento_importo_mensile as importo_mensile, finanziamento_mesi as mesi,     "
			...
			+ "  (select count(*) from bdom_t_graduatoria_dettaglio btgd2 where btgd2.graduatoria_id = (     "
			+ " 	select btg.graduatoria_id   " + " 	from bdom_t_graduatoria btg, bdom_r_graduatoria_stato brgs   "
			+ " 	where btg.graduatoria_id = brgs.graduatoria_id  " + " 	and btg.data_cancellazione is null  "
			...
			+ " 		and bts.sportello_cod = :codSportello)   "
			+ " ) and btgd2.finanziamento_id = btgf.finanziamento_id) as soggetti_finanziabili,       " + " case    "
			+ "	when btgf.area_id is not null  " + "	then ( " + "			select bda.area_desc  "
			+ "			from bdom_d_area bda " + "			where bda.area_id = btgf.area_id " + "	)   "
			+ "	when btgf.area_id is null  " + "	then null  " + "	else null   " + "end as area "
			+ " from bdom_t_graduatoria_finanziamento btgf  " + " where btgf.graduatoria_id  = (     "
			+ " 	select btg.graduatoria_id   " + " 	from bdom_t_graduatoria btg, bdom_r_graduatoria_stato brgs   "
			+ " 	where btg.graduatoria_id = brgs.graduatoria_id  " + " 	and btg.data_cancellazione is null  "
			+ " 	and brgs.data_cancellazione is null   "
			+ " 	and now() between coalesce(brgs.validita_inizio, now()) and coalesce(brgs.validita_fine,now())  "
			...
			+ " 		from bdom_t_sportello bts  " + " 		where bts.data_cancellazione is null  "
			+ " 		and bts.sportello_cod = :codSportello)   " + " )     "
			+ " and btgf.data_cancellazione is null     " + " group by btgf.finanziamento_id  "
			+ " order by coalesce(btgf.area_id,0) ASC ";

	/*
	 * Controllo stato graduatoria: - true, se lo stato della graduatoria e' uguale
	 * allo stato controllato
	 */
	private static String CHECK_STATO_GRADUATORIA = "select ( " + "	case  " + "		when ( "
			...
			+ "					from bdom_t_sportello bts " + "					where bts.data_cancellazione is null "
			+ "					and bts.sportello_cod = :codSportello) " + "				) "
			+ "		) = :statoGraduatoria " + "		then true " + "		else false " + "	end " + ") as controllo ";

	private static String CHECK_STATO_GRADUATORIA_BY_ID = "select ( " + "	case  " + "		when ( "
			+ "			select bdgs.graduatoria_stato_cod  "
			+ "			from bdom_r_graduatoria_stato brgs, bdom_d_graduatoria_stato bdgs "
			+ "			where bdgs.graduatoria_stato_id  = brgs.graduatoria_stato_id "
			+ " 		and now() between coalesce(brgs.validita_inizio, now()) and coalesce(brgs.validita_fine,now())"
	...
	private static String SELECT_GRADUATORIA_DESC = "select btg.graduatoria_desc as descrizioneGraduatoria, btg.graduatoria_cod as codiceGraduatoria, "
			+ "	bdgs.graduatoria_stato_desc as statoGraduatoria "
			+ "from bdom_t_graduatoria btg, bdom_r_graduatoria_stato brgs2, bdom_d_graduatoria_stato bdgs "
			+ "where btg.data_cancellazione is null    " + "and btg.graduatoria_id  = ( "
			+ "		select btg.graduatoria_id from bdom_t_graduatoria btg, bdom_r_graduatoria_stato brgs  "
			+ "		where btg.graduatoria_id = brgs.graduatoria_id " + "		and btg.data_cancellazione is null "
			+ "		and brgs.data_cancellazione is null  "
			+ "		and now() between coalesce(brgs.validita_inizio, now()) and coalesce(brgs.validita_fine,now()) "
	...
	private static String CHECK_PUBBLICAZIONE_GRADUATORIA = "select ( " + "	case  " + "		when ( "
			+ "			select count(*) as numero_domande "
			+ "			from bdom_t_domanda_dettaglio btdd, bdom_t_domanda btd  "
			+ "			where btdd.domanda_stato_id in ( " + "				select bdds.domanda_stato_id  "
			+ "				from bdom_d_domanda_stato bdds " + "				where bdds.domanda_stato_cod in ( "
			...
			+ "			and btdd.sportello_id = ( " + "				select bts.sportello_id  "
			+ "				from bdom_t_sportello bts  " + "				where bts.sportello_cod = :sportelloCod "
			+ "			) " + "			and btd.domanda_id = btdd.domanda_id "
			+ "			and btd.data_cancellazione is null " + "			and btdd.data_cancellazione is null "
			+ "			and now() between btdd.validita_inizio and coalesce (btdd.validita_fine, now()) "
			+ "			) > 0 " + "		then false " + "		else true " + "	end " + ") ";

	private static String UPDATE_R_GRADUATORIA_STATO = "UPDATE bdom_r_graduatoria_stato   "
	...
	private static String SELECT_DOMANDE_PUBBLICA_GRADUATORIA = "select " + "	btdd.domanda_det_id, "
			+ "	btd.domanda_numero, " + "	btd.richiedente_cf, " + "	btdd.sportello_id, " + "	case "
			+ "		when btdd.domanda_stato_id = (  " + "		select bdds.domanda_stato_id "
			+ "		from bdom_d_domanda_stato bdds  " + "		where bdds.domanda_stato_cod = (:stato1)  "
			+ "		)   " + "		then true  " + "		else false  " + "	end as riserva " + "from "
			+ "	bdom_t_domanda btd, " + "	bdom_t_domanda_dettaglio btdd, " + "	bdom_t_graduatoria_dettaglio btgd2 "
			+ "where " + "	btd.domanda_id = btdd.domanda_id " + "	and btdd.domanda_stato_id in (  "
			+ "		select bdds.domanda_stato_id " + "		from bdom_d_domanda_stato bdds  "
			+ "		where bdds.domanda_stato_cod = (:stato) " + "			or bdds.domanda_stato_cod = (:stato1)   "
			+ "		)  " + "	and btdd.domanda_id in ( " + "		select btgd.domanda_id  "
			+ "		from bdom_t_graduatoria_dettaglio btgd " + "		where btgd.graduatoria_id = ( "
			+ "			select btg.graduatoria_id " + "			from bdom_t_graduatoria btg, "
			+ "				bdom_r_graduatoria_stato brgs " + "			where btg.graduatoria_id = brgs.graduatoria_id "
			+ "			and btg.data_cancellazione is null  " + "			and brgs.data_cancellazione is null "
			+ "			and now() between coalesce(brgs.validita_inizio, now()) and coalesce(brgs.validita_fine, now()) "
			+ "			and btg.sportello_id in (  " + "				select bts.sportello_id "
			+ "				from bdom_t_sportello bts  "
			+ "				where bts.data_cancellazione is null and bts.sportello_cod = :codSportello)   "
			+ "			) " + "		and btgd.data_cancellazione is null   " + "	)  "
			+ "	and btgd2.finanziamento_id = ( " + "			select finanziamento_id   "
			+ "			from bdom_t_graduatoria_finanziamento btgf  " + "			where graduatoria_id = ( "
			+ "			select btg.graduatoria_id " + "			from bdom_t_graduatoria btg, "
			+ "				bdom_r_graduatoria_stato brgs " + "			where btg.graduatoria_id = brgs.graduatoria_id "
			+ "			and btg.data_cancellazione is null  " + "			and brgs.data_cancellazione is null "
			+ "			and now() between coalesce(brgs.validita_inizio, now()) and coalesce(brgs.validita_fine, now()) "
			...
			+ "				where bts.data_cancellazione is null and bts.sportello_cod = :codSportello)   "
			+ "			) " + "			and coalesce(area_id,0) = coalesce(:idArea,0) " + "	) "
			+ "	and btgd2.domanda_id  = btdd.domanda_id " + "	and btd.data_cancellazione is null "
			+ "	and btdd.data_cancellazione is null "
			+ "	and now() between btdd.validita_inizio and coalesce(btdd.validita_fine, now()) "
			+ "	and btgd2.data_cancellazione is null " + "	order by btgd2.posizione; ";

	private static String SELECT_GRADUATORIA_ID_BY_SPORTELLO_COD = "select btg.graduatoria_id from bdom_t_graduatoria btg, bdom_r_graduatoria_stato brgs  "
			+ "			where btg.graduatoria_id = brgs.graduatoria_id "
			+ "			and btg.data_cancellazione is null " + "			and brgs.data_cancellazione is null  "
			+ "			and now() between coalesce(brgs.validita_inizio, now()) and coalesce(brgs.validita_fine,now()) "
			+ "			and btg.sportello_id in ( " + "				select bts.sportello_id "
	...
	private static String SELECT_GRADUATORIA_ID_BY_SPORTELLO_ID = "select (case when (select count(btg.graduatoria_id) from bdom_t_graduatoria btg, bdom_r_graduatoria_stato brgs  "
			+ "			where btg.graduatoria_id = brgs.graduatoria_id "
			+ "			and btg.data_cancellazione is null " + "			and brgs.data_cancellazione is null  "
			+ "			and now() between coalesce(brgs.validita_inizio, now()) and coalesce(brgs.validita_fine,now()) "
	...
	private static String SELECT_GRADUATORIA_ID_BY_DOMANDA_DETTAGLIO_ID = "select (case when ((select count(*)  from bdom_t_graduatoria btg, bdom_t_graduatoria_dettaglio btgd, bdom_t_domanda_dettaglio btdd  "
			...
			+ "	and btdd.data_cancellazione is null "
			+ "	and btg.data_creazione = (select max(btg2.data_creazione) from bdom_t_graduatoria btg2 where btg2.data_cancellazione is null) "
			+ "	and btdd.domanda_det_id  =:idDetDomanda " + "	) " + "else 0 end ) ";

	private static String SELECT_IMPORTO_TOTALE_MENSILE_GRADUATORIA = "select (finanziamento_importo_mensile*finanziamento_mesi)as importo from bdom_t_graduatoria_finanziamento btgf  "
			+ "where graduatoria_id = :idGraduatoria " + "and area_id is null;";

	private static String DELETE_GRADUATORIA_DETTAGLIO = "delete from bdom_t_graduatoria_dettaglio "
	 ...
	 */
	private static String CHECK_ESISTENZA_GRADUATORIA = "select " + "	case " + "		when ( "
			+ "		select count(*) " + "		from bdom_t_graduatoria btg " + "		where btg.sportello_id = ( "
			+ "			select bts.sportello_id " + "			from bdom_t_sportello bts "
			+ "			where bts.sportello_cod = :codSportello  " + "			) "
			+ "		and btg.data_cancellazione is null " + "		) = 0  " + "		then false "
			+ "		else true " + "	end as controllo ";

	private static String UPDATE_IMPORTO_DOMANDE_GRADUATORIA1 = "update bdom_t_graduatoria_dettaglio  "
			+ "set importo = 0 " + "where graduatoria_id = :idGraduatoria; ";

	private static String UPDATE_IMPORTO_DOMANDE_GRADUATORIA2 = " update bdom_t_graduatoria_dettaglio  "
			+ "set importo = :importo  "
			+ "where domanda_id = (select btdd.domanda_id  from bdom_t_domanda_dettaglio btdd where btdd.domanda_det_id = :idDettaglio) "
	...
	public long insertTGraduatoria(String codSportello, String descGraduatoria, String utente)
		...
		try {
			...
			jdbcTemplate.update(INSERT_T_GRADUATORIA, params, keyHolder, new String[] { "graduatoria_id" });
			...
			for (int i = 0; i < ordinamenti.size(); i++) {
				if (ordinamenti.get(i).getOrdinamentoCampo().equals("domanda_data")) {
					...
					query += "  coalesce(CAST(" + ordinamenti.get(i).getOrdinamentoTabella()
							+ ".verifica_eg_punteggio_sociale AS numeric)," + "coalesce( "
							+ ordinamenti.get(i).getOrdinamentoTabella() + "."
	...
	public int[] updateImportoDomandeGraduatoria(double importo, List<ModelDomandeAmmissibili> domandeGraduatoria,
			Long gradId) throws DatabaseException {
