...
******************************************************************************/
package it.csi.buonodom.buonodombo.interceptor.server;
...
public class TraceRequestInterceptor extends LoggerUtil implements ContainerRequestFilter, ContainerResponseFilter {
	...
	public void filter(ContainerRequestContext requestContext) throws IOException {
		...
		try {
			...
			if (userInfo != null)
				logaudit.setUtente(userInfo.getCodFisc());
			else
			...
			if (httpRequest.getHeader("X-Request-ID") != null)
				logaudit.setUuid(httpRequest.getHeader("X-Request-ID"));
			else
				logaudit.setUuid(uuidAsString);
	...
	public void filter(ContainerRequestContext requestContext, ContainerResponseContext responseContext)
		...
		try {
			...
			try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
				/**
				 * TODO verificare se corretto prenderlo sempre o se fare un isJson come per la
				 * request
				 */
//					ObjectMapper mapper = new ObjectMapper();
//					mapper.writeValue(bos, entity);
				if (entity != null) {
					if (httpRequest.getPathInfo().contains("allegato") && responseContext.getStatus() == 200) {
						logaudit.setResponsePayload(null);
					} else {
